...
describe('public application flow', () => {
  it('should display the organization public page', () => {
    ...
    cy.findByRole('heading', { name: organization.name }).should('exist')
    cy.findByRole('heading', { name: organization.email }).should('exist')
    cy.findByRole('heading', { name: organization.phone }).should('exist')
